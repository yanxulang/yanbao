...
依赖审计默认拒绝 critical/high。任何抑制都必须精确指定诊断代码、复核原因和 UTC 到期日；过期项不会继续掩盖发现。项目事务在覆盖前保留恢复信息，并在检测到并发修改或无效日志时拒绝写回。正式发行档案均配有单 LF 的 SHA-256 文件，并由两套校验工具复验。请在公开披露前为修复预留合理时间。
