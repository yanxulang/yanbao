...
正式发行档案均配有单 LF 的 SHA-256 文件，并由两套校验工具复验。Release 同时提供 CycloneDX 1.5 SBOM、逐目标构建元数据、构建来源证明和 SBOM 证明；元数据绑定源码提交、目标、言序编译器、清单与锁文件摘要及独立应用摘要。发布流程在公开前验证证明，公开安装冒烟使用同一批已验签字节，正式化前任何一步失败都会保持预发布；已经稳定发布的 Release 不会被回退或覆盖。安装器只把`YANBAO_GITHUB_TOKEN`的私有副本交给 HTTPS 下载请求，不把该副本留在调用作用域，并在启动本机言序或发行程序前移除`GH_TOKEN`、`GITHUB_TOKEN`、`YANBAO_GITHUB_TOKEN`和`YANXU_GITHUB_TOKEN`。请在公开披露前为修复预留合理时间。
