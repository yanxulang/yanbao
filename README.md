...
- `update` / `更`：显式重新解析全部依赖；
- `list` / `列`：显示包和直接依赖；
- `run` / `行`：锁定依赖后调用`yanxu`按清单权限运行入口，并把`--`后的参数原样传入；
- 全局`--json`：输出`schema_version: 1`的机器结果；
- 事务变更：依赖解析失败时自动恢复清单和锁文件。

当前源码锁定言序核心`v1.1.4-A`：除继续继承安全制品解压、网络权限复查与套接字资源限制外，`run`还可使用核心新增的字节串、二进制文件/网络接口、`环境.参数`与锁定包模块加载边界。新项目模板继续分别声明外连、TCP 监听与 UDP 绑定权限。
...
yanbao --manifest-path 我的项目 run -- --once
```
...
`yanbao update`会丢弃旧锁定、刷新 Git 符号修订并重新选择版本；日常恢复项目应使用`yanbao install`，以优先验证现有锁定和缓存。Git 分支或标签最终都会在`言序.lock`中固定为精确提交。
